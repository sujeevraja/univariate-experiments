"""
    fo7()
``fo7'' is an MINLPLib test instance with binary and continuous variables
(see http://www.minlplib.org/fo7.html). The best known primal feasible
objective value for this problem is 20.72982507. This example contains 14 nonlinear
terms, all of which are of the form 1/x. 
"""

function post_fo7_linear_constraints!(m)
    ...
    @variable(m, y[1:14])
    @constraint(m, e199, 16*y[1]-x[94] <= 0.0)
    @constraint(m, e200, 16*y[2]-x[86] <= 0.0)
    @constraint(m, e201, 16*y[3]-x[95] <= 0.0)
    @constraint(m, e202, 16*y[4]-x[87] <= 0.0)
    @constraint(m, e203, 16*y[5]-x[96] <= 0.0)
    @constraint(m, e204, 16*y[6]-x[88] <= 0.0)
    @constraint(m, e205, 36*y[7]-x[97] <= 0.0)
    @constraint(m, e206, 36*y[8]-x[89] <= 0.0)
    @constraint(m, e207, 9*y[9]-x[98] <= 0.0)
    @constraint(m, e208, 9*y[10]-x[90] <= 0.0)
    @constraint(m, e209, 9*y[11]-x[99] <= 0.0)
    @constraint(m, e210, 9*y[12]-x[91] <= 0.0)
    @constraint(m, e211, 9*y[13]-x[100] <= 0.0)
    @constraint(m, e212, 9*y[14]-x[92] <= 0.0)
    y_indexes = collect(1:14)
    x_indexes = [86, 94, 87, 95, 88, 96, 89, 97, 90, 98, 91, 99, 92, 100]
    partitions = []
    for index in x_indexes 
        push!(partitions, [lower_bound(x[index]), upper_bound(x[index])])
    end 

    # @constraint(m, e199, 16/x[86]-x[94] <= 0.0)
    # @constraint(m, e200, 16/x[94]-x[86] <= 0.0)
    # @constraint(m, e201, 16/x[87]-x[95] <= 0.0)
    # @constraint(m, e202, 16/x[95]-x[87] <= 0.0)
    # @constraint(m, e203, 16/x[88]-x[96] <= 0.0)
    # @constraint(m, e204, 16/x[96]-x[88] <= 0.0)
    # @constraint(m, e205, 36/x[89]-x[97] <= 0.0)
    # @constraint(m, e206, 36/x[97]-x[89] <= 0.0)
    # @constraint(m, e207, 9/x[90]-x[98] <= 0.0)
    # @constraint(m, e208, 9/x[98]-x[90] <= 0.0)
    # @constraint(m, e209, 9/x[91]-x[99] <= 0.0)
    # @constraint(m, e210, 9/x[99]-x[91] <= 0.0)
    # @constraint(m, e211, 9/x[92]-x[100] <= 0.0)
    # @constraint(m, e212, 9/x[100]-x[92] <= 0.0)

    # ----- Objective ----- #
    @objective(m, Min, objvar)
    return y_indexes, x_indexes, partitions
end

function fo7()
    best_known_objective = 20.72982507
    cplex_optimizer =
    ...
    for i = 1:length(error_tolerances)
        ...
        y_indexes, x_indexes, base_partition = post_fo7_linear_constraints!(milp)
        # construct MILP relaxations
        for j = 1:length(y_indexes)
            ...
            @constraint(milp, var[x_index] == milp[:x][x_indexes[j]])
            @constraint(milp, var[y_index] == milp[:y][y_indexes[j]])
        end
    ...
    for i = 1:length(error_tolerances)
        ...
        y_indexes, x_indexes, base_partition = post_fo7_linear_constraints!(lp)
        # construct MILP relaxations
        for j = 1:length(y_indexes)
            ...
            @constraint(lp, var[x_index] == lp[:x][x_indexes[j]])
            @constraint(lp, var[y_index] == lp[:y][y_indexes[j]])
        end
